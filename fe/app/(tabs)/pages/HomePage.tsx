...
import { StyleSheet, View, SafeAreaView, Text, TouchableOpacity, ActivityIndicator, Image} from "react-native";
...
function Homepage() {
    ...
    let takePhoto = async () => {
        ...
        await cameraRef.current?.takePictureAsync().then((p:any) => {
            setPhoto(p);
            setUri(p.uri);
        })
    ...
    if ( photo ) {
        ...
        if (imgBase){
            ...
            img = <Image style={{height: '95%', width: '100%'}} source={{uri: uri!}} />
        }
    ...
    const toggleTypeCam = () => {
        setTypeCam((prev) => (prev === "back" ? "front" : "back"));
    }
    const getCam = () => {
        setIsCam(true)
    }
    ...
    const renderCamera = () => {
        return <CameraView style={styles.container} ref={cameraRef} facing={typeCam}>
